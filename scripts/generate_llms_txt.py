...
def get_example_files(examples_dir):
    ...
    for file_name in os.listdir(examples_dir):
        # Skip __init__.py, non-Python files, and demo.py
...
def extract_class_docstring(file_path):
    ...
    pattern = r'class\s+{}.*?:\s*[\'"]([^\'"]*)[\'"]'.format(class_name)
...
def extract_main_example(file_path, _):
...
def extract_init_ui_method(file_path, _):
    """Extract the _init_ui method from a class."""
    with codecs.open(file_path, encoding="utf-8") as f:
        content = f.read()

    # Find the _init_ui method
    pattern = r"def\s+_init_ui\s*\(\s*self.*?\):(.*?)(?=\n\s*def|\n\s*if\s+__name__|$)"
    init_ui_match = re.search(pattern, content, re.DOTALL)
    if init_ui_match:
...
def generate_llms_txt(examples_dir, output_file):
    ...
    for file_name in example_files:
        ...
        if main_code:
            ...
            # Use docstring if available, otherwise create a generic description
            description = (docstring or
                          f"Example of how to use the {class_name} widget from dayu_widgets library.")
        ...
        if init_ui_code:
            ...
            # Create a description for the UI initialization example
            description = (f"Example of how to initialize and configure the UI "
                          f"for {class_name} widget.")
            source = f"{REPO_URL}/blob/master/examples/{file_name}#L1"
    ...
    import_snippet = format_code_snippet(
        ...
        "This code snippet demonstrates how to import the dayu_widgets library into a Python script. "
        "This import statement is the prerequisite for using any of the UI components provided by "
        "the library.",
...
def main():
    parser = argparse.ArgumentParser(
        description="Generate llms.txt file for Context7")
