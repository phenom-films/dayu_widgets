...
def custom_folder(tmpdir_factory):
...
def test_get_custom_append_static_folder(custom_folder):
    ...
    for input_file, result in (
            ...
            (os.path.join(os.path.dirname(__file__), 'for_test.txt'),
             os.path.join(os.path.dirname(__file__), 'for_test.txt')),  # user give a full path file, return
            ('sub_folder/sub_file.png', os.path.join(str(custom_folder), 'sub_folder/sub_file.png')),
    ):
