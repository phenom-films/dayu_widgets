...
from examples.alert_example import AlertExample
import pytest

# Import local modules
from dayu_widgets import MAlert
...
class TestAlertExample(object):
    ...
    def test_setup_property(self):
        """Test if we can get the properties that have already been set."""
        assert self.view.windowTitle() == "Example for MAlert"

    @pytest.mark.parametrize(
        "kwargs,result", [(i, r) for i, r in zip(TEST_INPUT, TEST_OUTPUT)]
    )
    def test_malert_init(self, qtbot, kwargs, result):
    ...
    def test_malert_with_wrong_type(self, qtbot, input_type):
